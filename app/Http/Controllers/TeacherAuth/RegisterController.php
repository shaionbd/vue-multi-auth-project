...
class RegisterController extends Controller
{
    ...
    */
    ...
    protected function create(array $data)
    {
        ...
        $teacher->company = $data['company'];
        $teacher->password = bcrypt($data['password']);
        
        $teacher->save();

        // send activation email
        $activation_url = 'http://localhost:8000/teacher/account/activate?id='.$teacher->id.'&curr_id='.$teacher->password;
        $email = new \SendGrid\Mail\Mail();
        $email->setFrom("hello@simonechinaglia.net");
        $email->setSubject("Account Activation");
        
        $email->addTo($teacher->email, "");
        $email->addContent(
    ...
    public function activate(Request $request){
        ...
        $teacher = Teacher::where('id', $id)->where('password', $password)->first();
        $teacher->is_activate = 1;
        $teacher->save();

        // return response()->json(['id'=> $id, 'pass'=> $password]);
